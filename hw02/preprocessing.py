...
def get_mfccs(filename, downsampling_rate, frame_length_in_s, frame_step_in_s, num_mel_bins, lower_frequency, upper_frequency, num_coefficients):
    ...
    mfccs = tf.signal.mfccs_from_log_mel_spectrograms(log_mel_spectrogram)[..., :num_coefficients]  # select only the first "num_coefficients" coefficients
                                                                                                    # same as using [:, :num_coefficients]

    # mfccs = tf.signal.mfccs_from_log_mel_spectrograms(log_mel_spectrogram)[:, 1:num_coefficients]  # The reasons for discarding the other coefficients is that they represent 
                                                                                                   # fast changes in the filter bank coefficients and these fine details 
                                                                                                   # don’t contribute to Automatic Speech Recognition                                                                                               
